-- =============================================================================
-- إصلاح نظام التحويلات - معالجة الأخطاء والتحسينات
-- تاريخ الإنشاء: 2025-01-08
-- =============================================================================

-- إصلاح دالة معالجة التحويلات الرئيسية
...
BEGIN
    normalized_identifier := LOWER(TRIM(identifier));
    
    -- البحث الدقيق بالبريد الإلكتروني
    RETURN QUERY
    SELECT 
        u.id as user_id,
        u.email,
        u.full_name,
        u.account_number,
        COALESCE(ud.can_receive_transfers, true) as can_receive,
        'exact_email'::TEXT as match_type
    FROM public.users u
    LEFT JOIN public.user_directory ud ON u.id = ud.user_id
    WHERE LOWER(TRIM(u.email)) = normalized_identifier
    ...
    IF NOT FOUND THEN
        ...
        FROM public.users u
        LEFT JOIN public.user_directory ud ON u.id = ud.user_id
        WHERE UPPER(TRIM(u.account_number)) = UPPER(TRIM(identifier))
          AND COALESCE(u.is_active, true) = true
        LIMIT 1;
    END IF;
...
-- إصلاح دالة التحقق من حدود التحويل
